...
import { Cloud, Database, Zap, Workflow, Code2, Server, Cpu } from 'lucide-react';
...
const fadeUp = {
  ...
  visible: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: { duration: 0.5, delay: i * 0.1, type: 'spring' as const, bounce: 0.3 },
  }),
};
...
export function About() {
  ...
  const cards = [
    { icon: Cloud, title: t.about.card1Title, desc: t.about.card1Desc, color: 'cyber' },
    { icon: Database, title: t.about.card2Title, desc: t.about.card2Desc, color: 'mint' },
    { icon: Zap, title: t.about.card3Title, desc: t.about.card3Desc, color: 'crimson' },
    { icon: Workflow, title: t.about.card4Title, desc: t.about.card4Desc, color: 'cyber' },
  ];
          ...
          >
            ...
            <div className="relative z-10 flex h-full flex-col">
              <div className="mb-4 flex items-center gap-3">
                ...
                <span className="font-mono text-sm font-bold text-cyber">emot</span>
              </div>
              <p className="text-sm leading-relaxed text-slate-300 sm:text-base">{t.about.bio1}</p>
              <p className="mt-4 text-sm leading-relaxed text-slate-400">{t.about.bio2}</p>
              <p className="mt-4 text-sm leading-relaxed text-slate-400">{t.about.bio3}</p>

              {/* Matrix nodes decoration */}
              <div className="mt-auto pt-6">
                <div className="flex flex-wrap gap-2">
                  {['React', 'TypeScript', 'Supabase', 'PostgreSQL', 'Node.js', 'Cloud', 'Docker'].map(
                    (tech) => (
                      <span
                        key={tech}
          ...
          {cards.map((card, i) => {
            ...
            return (
              <motion.div
                ...
                whileInView="visible"
                viewport={{ once: true, margin: '-50px' }}
                className="glass-card clip-corner group relative aspect-square overflow-hidden p-5 transition-transform hover:scale-[1.02] sm:aspect-auto"
              >
                ...
                <p className="mt-2 text-xs leading-relaxed text-slate-400 sm:text-sm">{card.desc}</p>
              </motion.div>
            );
          })}
        </div>
